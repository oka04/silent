    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if (!is_readonly)
            {
				...
				////TextW‚É“ü‚Á‚Ä‚¢‚é‚Ì‚ÍSJISATextA‚É“ü‚ê‚é•¶Žš‚ðUTF8‚É•ÏŠ·‚·‚é‚±‚Æ‚ÅInputText‚Ì“ú–{Œê‚ª³í‚É•\Ž¦‚³‚ê‚é
				if (*state->TextW.Data) {
					ImTextStrToUtf8(state->TextA.Data, state->TextA.Size, state->TextW.Data, NULL);
					{
						std::string work = state->TextA.Data;
						work = SjistoUTF8(work);
						int len = work.size();
						if (state->TextA.Size < len + 1) {
							state->TextA.resize(len + 1);
						}
						strcpy_s(state->TextA.Data, len + 1, work.c_str());
						state->CurLenA = strlen(state->TextA.Data);
					}
				}
            }
